...
<img src="https://render.githubusercontent.com/render/math?math=\mathrm{f}_v(\dot{q}) = \sum\limits_{n=1}^{M}F_{v,n}\,b_n,\quad b_n = \begin{matrix}|\dot{q}|\,\dot{q}^{n-1}, & \text{if } n \text{ is even}\\ \dot{q}^n, & \text{otherwise}\end{matrix}\dots">
